...
/**
 * Normalize the conversation_id from a chat.message event so it uses the
 * Mission Control UI format ("agent_<name>") even when the event was
 * generated by OpenClaw with a session-key-format ID ("agent:name:type").
 */
function normalizeChatConversationId(data: any): string {
  ...
  if (data?.from_agent && data.from_agent !== 'human' && data.from_agent !== 'system') {
    return `agent_${data.from_agent}`
  }
      ...
      switch (event.type) {
        ...
        case 'chat.message':
          if (event.data?.id) {
            addChatMessage({
              id: event.data.id,
              conversation_id: normalizeChatConversationId(event.data),
